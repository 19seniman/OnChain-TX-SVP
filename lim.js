...
// Default base routing token: alamat ini adalah _WSVP asli dari constructor argument
// router (terkonfirmasi dari kontrak terverifikasi), bukan token WETH biasa.
let ROUTER_WETH_ADDRESS = "0x771a0a63D8198b7dbea4a16910ff68AB38006531";
...
async function main() {
    try {
        ...
        try {
            ROUTER_WETH_ADDRESS = await routerContract.WSVP();
            console.log(`${getTime()} ${C.green}🔗 Terhubung. Base Route Terdeteksi via WSVP(): ${ROUTER_WETH_ADDRESS}${C.reset}\n`);
        } catch (e1) {
            try {
                ROUTER_WETH_ADDRESS = await routerContract.WETH();
                console.log(`${getTime()} ${C.green}🔗 Terhubung. Base Route Terdeteksi via WETH(): ${ROUTER_WETH_ADDRESS}${C.reset}\n`);
            } catch (e2) {
                console.log(`${getTime()} ${C.yellow}⚠️ Router tidak punya fungsi WSVP() maupun WETH(). Menggunakan default manual: ${ROUTER_WETH_ADDRESS}${C.reset}\n`);
            }
        }
